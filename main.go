...
func main() {
	cfg, err := LoadConfig("/home/tamal/go/src/github.com/kubeshield/scanner/hack/deploy/clair/config.yaml")
	if err != nil {
		log.Fatalln(err)
	}

	fmt.Println(cfg.API.CAFile)
	fmt.Println(cfg.API.CertFile)
	fmt.Println(cfg.API.KeyFile)
}